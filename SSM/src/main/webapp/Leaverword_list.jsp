...
<c:if test="${leavewords == null }">
	<c:redirect url="findAll_lw"></c:redirect>
</c:if>
...
<body>
	...
	<div class="rightinfo">
		...
		<table class="tablelist">
			...
			<tbody>
				<c:forEach items="${leavewords }" var="lw" varStatus="sta">
					<tr>
						<td><input name="checkbox" type="checkbox" value="${lw.leaveWordId }" /></td>
						<td>${sta.count }</td>
						<td>${lw.user.userName }</td>
						<td>${lw.leaveWordMessage }</td>
						<td>${lw.leaveWordTimeString }</td>
						<td><a href="del_lw?id=${lw.leaveWordId }" class="tablelink"> 删除</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
