...
<c:if test="${adm == null }">
	<c:redirect url="findById_adm"></c:redirect>
</c:if>
...
<body>
	...
	<div class="rightinfo">
		...
		<table class="tablelist">
			...
			<tbody>
				<tr>
					<td><input name="checkbox" type="checkbox" value="${adm.adminId }" /></td>
					<td>${sta.count }</td>
					<td>${adm.adminName }</td>
					<td>${adm.adminGender }</td>
					<td>${adm.adminTel }</td>
					<c:if test="${adm.adminGrade==1 }">
						<td>超级管理员</td>
					</c:if>
					<c:if test="${adm.adminGrade==2 }">
						<td>管理员</td>
					</c:if>
					<c:if test="${adm.adminGrade==2 }">
						<td><a href="del_admin?id=${adm.adminId }" class="tablelink"> 删除</a></td>
					</c:if>
				</tr>
			</tbody>
		</table>
